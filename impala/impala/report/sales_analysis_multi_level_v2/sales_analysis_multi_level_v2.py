...
def group_level_1_1(conditions = "" , grouping_level_1 = None,  grouping_level_value  = None, range_by = None , filters = None):
	if grouping_level_1:
		...
		data =  frappe.db.sql(""" select  {} as level_1,  s.posting_date, s.name  
			...
			from `tabSales Invoice` s
			INNER join 	`tabSales Invoice Item` i  on s.name = i.parent
			where s.docstatus=1 and {} = "{}" {}   order by 1 """.format(numbering, numbering.replace('"', '').strip(), grouping_level_value.replace('"', '').strip() if grouping_level_value else "NULL", conditions, sales_person= grouping_level_value), as_dict=1, debug=False)
	return data
		...
		if grouping_level_2:
			...
			data =  frappe.db.sql(""" select  {}   as level_1 , {} as level_2 , 
				...
				order by {}   ,  {} , YEAR(s.posting_date) asc """.format( numbering_1 ,  numbering_2 , ranges , numbering , 
					...
					numbering_2.replace('"', '').strip() , grouping_level_value_2.replace('"', '').strip() if grouping_level_value_2 else "NULL" ,    conditions , grouping_1 , grouping , numbering_1 , numbering), as_dict=1, debug=False)		
			...
			if grouping_level_3:
				...
				data =  frappe.db.sql(""" select  {}   as level_1 , {} as level_2 , {} as level_3,  
					...
					order by {}   ,  {} , YEAR(s.posting_date) asc """.format( numbering_1 ,  numbering_2 , numbering_3 ,  ranges , numbering , 
						...
						conditions , grouping_1 , grouping , numbering_1 , numbering), as_dict=1, debug=False)		
