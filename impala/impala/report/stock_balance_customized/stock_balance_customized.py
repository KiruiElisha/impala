	...
	for (company, item, warehouse) in sorted(iwb_map):
		if item_map.get(item):
			...
			if include_uom:
				...
				report_data['conversion_factor'] = item_map[item].conversion_factor
...
def get_columns(filters):
	...
	columns = [
		...
		{"label": _("Item Thickness"), "fieldname": "item_thickness", "fieldtype": "Link", "options": "Item Thickness", "width": 100},
		...
		{"label": _("Color"), "fieldname": "color","width": 150},
	...
	if filters.get('include_uom'):
		columns +=[{"label": _("Conversion Factor"), "fieldname": "conversion_factor", "fieldtype": "Data","width": 100}]
...
def get_stock_ledger_entries(filters, items):
    ...
    item_category = filters.get("item_category")
    color = filters.get("color")  # Get color from filters
    item_thickness=filters.get("item_thickness")
    ...
    # Get brand from tabItem
    brand_conditions_sql = ''
    if brand:
        brand_conditions_sql = f"""
            AND sle.item_code IN (
                SELECT name FROM `tabItem` WHERE brand LIKE '%%{brand}%%'
            )
        """
    
    # Get item_category from tabItem
    category_conditions_sql = ''
    if item_category:
        category_conditions_sql = f"""
            AND sle.item_code IN (
                SELECT name FROM `tabItem` WHERE item_category LIKE '%%{item_category}%%'
            )
        """

    # Get color from tabItem
    color_conditions_sql = ''
    if color:
        color_conditions_sql = f"""
            AND sle.item_code IN (
                SELECT name FROM `tabItem` WHERE color LIKE '%%{color}%%'
            )
        """

    item_thickness_conditions_sql = ''
    if item_thickness:
        item_thickness_conditions_sql = f"""
            AND sle.item_code IN (
                SELECT name FROM `tabItem` WHERE item_thickness LIKE '%%{item_thickness}%%'
            )
        """
    ...
    return frappe.db.sql("""
        ...
        WHERE sle.docstatus < 2 %s %s %s %s %s
        AND is_cancelled = 0
        ORDER BY sle.posting_date, sle.posting_time, sle.creation, sle.actual_qty""" % #nosec
        (item_conditions_sql, conditions, brand_conditions_sql, category_conditions_sql, color_conditions_sql), as_dict=1, debug=True)
...
def get_items(filters):
	...
	if filters.get("disabled"):
		...
		conditions.append("(item.disabled = 0 OR item.disabled IS NULL)")

	items = []
	if conditions:
		items = frappe.db.sql_list("""select name,disabled from `tabItem` item where {}"""
...
def get_item_details(items, sle, filters):
	...
	res = frappe.db.sql("""
		select
			item.name, item.item_name, item.description,item.item_category,item.color,item.item_thickness, item.item_group, item.brand, item.stock_uom %s
		from
			`tabItem` item
			%s
		where
