	...
	for g in group_by_data:
		...
		data_sum = frappe.db.sql(""" select c.employee, SEC_TO_TIME(SUM(TIME_TO_SEC(total_time))) AS total_time , 
			SEC_TO_TIME(SUM(TIME_TO_SEC(overtime_time))) AS overtime_time, c.employee_name
			  ...
			  where c.docstatus =1 {} and e.{} = '{}' group by 1 order by c.attendance_date""".format(conditions, field_is, g.name), as_dict=1, debug=False)
		if data_sum:
...
def get_details(conditions="", filters={}):
	details = {}
	# '<input type="text" value="00:00:00" class="arrival_time" style="width:80px; text-align: center;  color: black;">' as arrival_time,
	data = frappe.db.sql("""select e.name, e.employee_name, attendance_date as date,
	 		time_in, time_out,

	 		CONCAT('<input type="text" value="', time_in,'" class="arrival_time" data-employee="', e.name,'"data-employee_name="', e.employee_name ,
			'" data-date="', attendance_date,
			'" data-time_table="', c.employee_shift,
			'" data-checkout_time="', CONCAT(attendance_date, ' ', time_out),
			'" style="width:100px; text-align: center;  color: black;">') as arrival_time,

			CONCAT('<input type="text" value="', time_out,'" class="departure_time" data-employee="', e.name,'"data-employee_name="', e.employee_name ,
			'" data-date="', IF(t.is_double_date_shift, ADDDATE(attendance_date, 1), attendance_date),
			'" data-time_table="', c.employee_shift,
			'" data-checkout_time="', CONCAT(IF(t.is_double_date_shift, ADDDATE(attendance_date, 1), attendance_date), ' ', time_out),
			'" style="width:100px; text-align: center;  color: black;">') as departure_time,
	 		
	 		total_time, c.employee_shift as time_table,
			...
			from `tabEmployee` e inner join `tabAttendance` c on c.employee = e.name inner join `tabTime Table` t on c.employee_shift = t.name
			where e.status = 'Active' and c.docstatus = 1 {} order by e.employee_name,attendance_date
		""".format(conditions), as_dict=1, debug=False)
	print(data)
	return data
