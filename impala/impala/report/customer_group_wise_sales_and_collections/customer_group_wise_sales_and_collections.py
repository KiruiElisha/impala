...
def get_daily_sales(select_group, conditions, groupby):
    ...
    sales = frappe.db.sql(
        """SELECT {}, CONCAT(currency, " ", FORMAT(SUM(grand_total),2)) as sales
...
def get_daily_collections(conditions, groupby):
    ...
    collections = frappe.db.sql(
        """ SELECT pe.party, customer_group as details, CONCAT(paid_from_account_currency," ", FORMAT(SUM(pe.paid_amount),2)) as collection
