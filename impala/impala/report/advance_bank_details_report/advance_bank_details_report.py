...
def get_data(conditions="", filters={}):
	...
	SQL_QUERY = """
		...
		FROM
			`tabPayroll Monthly Input` mpi
			INNER JOIN `tabPayroll Monthly Input Employees` mpid ON mpi.name=mpid.parent
			INNER JOIN `tabEmployee` emp ON mpid.employee = emp.name
		WHERE mpi.docstatus<2 and mpi.salary_component='Advances' %s
	""" %conditions
...
def get_conditions(filters):
    ...
    payroll_categories = frappe.db.get_list("User Permission",filters={'user': frappe.session.user,'allow': "Payroll Category"},fields=["for_value"])
    if payroll_categories:
        payroll_categories = [p['for_value'] for p in payroll_categories]
        payroll_category_conditions = [f"emp.payroll_category = '{category}'" for category in payroll_categories]
        conditions += " AND (" + " OR ".join(payroll_category_conditions) + ")"

    if filters.get("from_date"):
        conditions += " and mpi.payroll_date >= '{}'".format(filters.get("from_date"))
    if filters.get("to_date"):
        conditions += " and mpi.payroll_date <= '{}'".format(filters.get("to_date"))
