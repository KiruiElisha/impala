...
def get_data(filters, conditions, group):
    data = frappe.db.sql("""
        ...
        CONCAT("<span style='margin-left: 20px'>", sii.item_code, "</span>") as item_code, sii.description,
        sii.qty, sii.base_net_amount, item.item_category, item.item_group, st.sales_person,
        ...
        `tabStock Ledger Entry`.voucher_no = IFNULL(IF(si.is_return=0, IF(si.update_stock=0, sii.delivery_note, sii.parent), sii.delivery_note),
        ...
        FROM `tabSales Invoice View` AS si 
        INNER JOIN `tabSales Invoice Item View` AS sii ON si.name = sii.parent 
        LEFT JOIN `tabItem` AS item ON sii.item_code = item.name 
        LEFT JOIN `tabSales Team` AS st ON st.parent = sii.parent
        LEFT JOIN `tabDelivery Note` AS dn ON dn.name = sii.delivery_note
        WHERE si.docstatus = 1 AND {} = "{}" {} AND dn.status = 'Closed'
    """.format(
        "si." + frappe.scrub(filters.groupby) if filters.groupby not in ('Item Group', 'Warehouse') else "sii." + frappe.scrub(filters.groupby),
        group,
