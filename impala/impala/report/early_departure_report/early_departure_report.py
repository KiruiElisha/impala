...
def execute(filters=None):
	...
	for g in group_by_data:
		...
		data_sum = frappe.db.sql(""" select c.employee , 
			SEC_TO_TIME(SUM(TIME_TO_SEC(early_exit_time))) AS early_exit_time, c.employee_name
			  from `tabAttendance` c inner join `tabEmployee` e on e.name = c.employee
			  where c.docstatus =1 {} and early_exit =1 and e.{} = '{}' group by 1 order by c.attendance_date""".format(conditions, field_is, g.name), as_dict=1, debug=True)
		if data_sum:
		...
		for d in data_sum:
			...
			if master:
				...
				total.update({"early_exit_time": correct_time(d.early_exit_time)})
				data.append(total)
				data.append({})
...
def get_details(conditions="", filters={}):
	...
	data = frappe.db.sql("""select e.name, e.employee_name, attendance_date as date, time_in, time_out, total_time, c.employee_shift as time_table,
			...
			where e.status = 'Active' and c.docstatus = 1 and c.early_exit=1 {} order by e.employee_name,attendance_date
