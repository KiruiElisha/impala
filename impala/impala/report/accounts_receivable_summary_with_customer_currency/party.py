...
def set_address_details(
    ...
    if party_type in ["Customer", "Lead"]:
        party_details.shipping_address_name = (
            shipping_address or get_party_shipping_address(party_type, party.name)
        )
    ...
    if doctype and doctype in ["Delivery Note", "Sales Invoice", "Sales Order"]:
        ...
        if party_details.company_address:
            party_details["shipping_address"] = (
                shipping_address or party_details["company_address"]
            )
...
def get_default_price_list(party):
    ...
    if party.doctype == "Customer":
        price_list = frappe.get_cached_value(
            "Customer Group", party.customer_group, "default_price_list"
        )
        if price_list:
            return price_list
...
def get_party_account(party_type, party=None, company=None):
    ...
    if not account and party_type in ["Customer", "Supplier"]:
        party_group_doctype = (
            "Customer Group" if party_type == "Customer" else "Supplier Group"
        )
...
def get_party_account_currency(party_type, party, company):
    def generator():
        ...
        return frappe.db.get_value(
            "Account", party_account, "account_currency", cache=True
        )

    return frappe.local_cache(
        "party_account_currency", (party_type, party, company), generator
    )
...
def validate_party_gle_currency(
    party_type, party, company, party_account_currency=None
):
...
def validate_party_accounts(doc):
    ...
    for account in doc.get("accounts"):
        ...
        if (
            doc.get("default_currency")
            and party_account_currency
            and company_default_currency
        ):
            if (
    ...
    if getdate(due_date) < getdate(posting_date):
        ...
        if default_due_date != posting_date and getdate(due_date) > getdate(
            ...
            is_credit_controller = (
                frappe.db.get_single_value("Accounts Settings", "credit_controller")
                in frappe.get_roles()
            )
...
def get_address_tax_category(
    tax_category=None, billing_address=None, shipping_address=None
):
    ...
    if addr_tax_category_from == "Shipping Address":
        if shipping_address:
            tax_category = (
                frappe.db.get_value("Address", shipping_address, "tax_category")
                or tax_category
            )
    else:
        if billing_address:
            tax_category = (
                frappe.db.get_value("Address", billing_address, "tax_category")
                or tax_category
            )
...
def get_payment_terms_template(party_name, party_type, company=None):
    ...
    if party_type == "Customer":
        customer = frappe.get_cached_value(
            "Customer",
            party_name,
            fieldname=["payment_terms", "customer_group"],
            as_dict=1,
        )
        ...
        supplier = frappe.get_cached_value(
            "Supplier",
            party_name,
            fieldname=["payment_terms", "supplier_group"],
            as_dict=1,
        )
    ...
    if party_type and party_name:
        if party_type in ("Customer", "Supplier"):
            party = frappe.get_cached_value(
                party_type, party_name, ["is_frozen", "disabled"], as_dict=True
            )
            if party.disabled:
                frappe.throw(
            ...
            if frappe.db.get_value("Employee", party_name, "status") != "Active":
                frappe.msgprint(
                    _("{0} {1} is not active").format(party_type, party_name),
                    alert=True,
                )
...
def get_dashboard_info(party_type, party, loyalty_program=None):
    ...
    companies = frappe.get_all(
        doctype,
        filters={"docstatus": 1, party_type.lower(): party},
        distinct=1,
        fields=["company"],
    )
    ...
    for d in companies:
        ...
        if party_account_currency == company_default_currency:
            billing_this_year = flt(
                company_wise_billing_this_year.get(d.company, {}).get(
                    "base_grand_total"
                )
            )
        else:
            billing_this_year = flt(
                company_wise_billing_this_year.get(d.company, {}).get("grand_total")
            )
