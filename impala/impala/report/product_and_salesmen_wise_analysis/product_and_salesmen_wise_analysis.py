...
def execute(filters=None):
	...
	if filters.get("level_1") and filters.get("level_1") != "":
		...
		if level_1:
			if not filters.get("level_2")  and  not filters.get("value") and not filters.get("range"):
				for i in level_1:
			...
			elif filters.get("level_1") and  filters.get("level_2") and  not filters.get("value") and not filters.get("range"):
...
def group_level_2(conditions = "" , grouping_level_1  = "", grouping_level_value_1 = "" , grouping_level_2  = ""):
	...
	if grouping_level_1:
		...
		if grouping_level_2:
			...
			where 1=1  and {} = '{}' {}  {} , {} order by s.name desc 
...
def get_values_column( conditions = ""  , filters = ""):
	...
	if filters.get("value") and filters.get("value") != "":
		...
		if filters.get("value") == "return_inclusive":
			values+= """  , 
				IF( s.tax_type= 'Inclusive' , 
					i.amount -  (i.amount *  i.total_tax_percentage  / 100)   , 
					0
		...
		if filters.get("value") == "Reetun Exclusive":
			values+= """  , 
					IF( s.tax_type= 'Exclusive' , 
					(i.amount *  i.total_tax_percentage  / 100) + i.amount  , 
					i.amount
		...
		if filters.get("value") == "Return To Sales":
			values+= """  , 
					IF( s.tax_type= 'Exclusive' , 
					(i.amount *  i.total_tax_percentage  / 100) + i.amount  , 
					i.amount
