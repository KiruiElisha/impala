...
class PartyLedgerSummaryReport(object):
	...
	def get_data(self):
		...
		for gle in self.gl_entries:
			customer_pdcheques = frappe.db.sql("""select name, base_amount
				from `tabPosted Dated Cheques`
				where company='{}' and party_type = 'Customer' and party = '{}' and posting_date BETWEEN '{}' and '{}' 
				""".format(self.filters.company, gle.party, self.filters.get("from_date"), self.filters.get("to_date")), as_dict=1)

			pdcheques = ""
			outstanding_amount = 0.0
			
			for pd in customer_pdcheques:
