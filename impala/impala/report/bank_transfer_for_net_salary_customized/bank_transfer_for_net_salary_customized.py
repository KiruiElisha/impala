...
def get_data(conditions="", filters={}):
	...
	personal_email = frappe.get_value("User", user_id, "email")
	company_email = frappe.get_value("User", user_id, "company_email")

    # Use the first non-empty value among user_id, personal_email, and company_email as the key
	key_field = next((field for field in [user_id, personal_email, company_email] if field), None)

    # Fetch employee details using the key field
	# employee_data = frappe.get_value("Employee", {key_field: key_field}, ["employee_number", "payroll_category"], as_dict=True)
	employee_data = frappe.get_all("Employee", filters={"user_id": user_id}, fields=["employee_number", "payroll_category"])


	if employee_data:
		employee_number = employee_data.get("employee_number")
		payroll_category = employee_data.get("payroll_category")

        # Use the payroll_category in your logic or modification

        # Fetch other data based on conditions
		SQL_QUERY = f"""
			SELECT
				sslip.net_pay,
				sslip.docstatus,
			...
			WHERE sslip.docstatus<2 {conditions}
		"""
		...
		print(f"No employee found for user {user_id}, personal email {personal_email}, or company email {company_email}")
		return []
...
def get_user_payroll_category(user_id, personal_email, company_email):
    # Use the first non-empty value among user_id, personal_email, and company_email as the key
    key_field = next((field for field in [user_id, personal_email, company_email] if field), None)

    # Fetch employee details using the key field
    employee_data = frappe.get_value("Employee", {key_field: key_field}, ["payroll_category"], as_dict=True)

    return employee_data.get("payroll_category", None)
