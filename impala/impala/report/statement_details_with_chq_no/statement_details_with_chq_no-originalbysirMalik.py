...
def execute(filters=None):
    ...
    if cheque_list_detail:
        ...
        totals_after_pd.update(
            {"pd_reference_no": "Due Amount After PDs", "p_paid_amount": total_final_dues_after_pd}
        )
    ...
    if filters.get("party_type") and filters.get("party"):
        ...
        if customer_detail:
            ...
            customer_personal_detial["currency"] = frappe.db.get_value(
                "Company", {"name": filters.get("company")}, "default_currency"
            )
...
def get_accountwise_gle(filters, accounting_dimensions, gl_entries, gle_map, ageing_data):
...
def get_pd_cheque_detail(filters):
   ...
   #          """ 
            ...
            # where pd.docstatus=1 and pd.status='Pending' and pd.party_type = '{}' and pd.party = '{}' and pd.posting_date >= '{}'

         # """.format(
   #              filters.get("party_type"), party, getdate(nowdate())
   #          ),
   #          as_dict=1,
   #          debug=1,
   #      )
        ...
        #     where 1=1 and status='Pending' and party_type = '{}' and party = '{}' and posting_date >= '{}'
        ...
        cheque_list = frappe.db.sql(
            ...
            base_amount as p_paid_amount
            from `tabPosted Dated Cheques`
            where 1=1 and status='Pending' and party_type = '{}' and party = '{}'
