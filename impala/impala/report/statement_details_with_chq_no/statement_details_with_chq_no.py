    ...
    if filters.get("party_type") and filters.get("party"):
        ...
        if filters.get("party_type") == "Supplier":
            ...
            customer_detail = frappe.db.get_value(
                "Supplier",
                {"name": party},
                ["supplier_name", "supplier_primary_address", "company", "default_currency"],
                as_dict=1,
            )
            if customer_detail:
            ...
            customer_detail = frappe.db.get_value(
                "Customer",
                {"name": party},
                ["customer_name", "customer_primary_address", "company", "default_currency"],
                as_dict=1,
            )
...
def get_gl_entries(filters, accounting_dimensions):
    ...
    if filters and filters.get("cost_center"):
        ...
        SELECT name as gl_entry,
            ...
            account_currency,
            remarks, against,
            is_opening, `tabGL Entry View`.creation {select_fields_with_percentage}
        FROM `tabGL Entry View`,
        (
...
def get_result_as_list(data, filters):
    ...
    for d in data:
        ...
        if d.get("voucher_type") == "Sales Invoice":
            ...
            payment_vals = frappe.db.get_value("Payment Entry", {"name": d.get("voucher_no")}, ["ref_no", "reference_date", "mode_of_payment", "reference_no"],as_dict=True)
