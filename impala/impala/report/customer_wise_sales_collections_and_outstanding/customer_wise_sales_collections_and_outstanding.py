...
def execute(filters=None):
    ...
    select_group = ""
    groupby = ""
    select_group = "customer as details"
    data = []
    ...
    for d in master:
        ...
        for i in range(12):
            groupby = "GROUP BY customer ORDER BY customer"
            ...
            groupby = "GROUP BY cs.customer_name ORDER BY cs.customer_name"
...
def get_details(filters):
    if filters.get("customer"):
        details = frappe.db.sql(
            """SELECT name as details FROM `tabCustomer`
				WHERE disabled=0 and  company = '{}' and name = '{}' """.format(
                filters.get("company"),
...
def get_monthly_sales(select_group, details, month, year, groupby):
    sales = frappe.db.sql(
        """SELECT {}, SUM(base_grand_total) as sales
                    ...
                    WHERE docstatus=1 and customer="{}" and MONTHNAME(posting_date)='{}' and YEAR(posting_date) = '{}' {}""".format(
            select_group,
            details,
            month,
            year,
...
def get_monthly_collections(details, month, year, groupby):
    collections = frappe.db.sql(
        """SELECT cs.name as details, SUM(pe.base_paid_amount) as collection
        ...
        inner join `tabCustomer` cs on  pe.party = cs.customer_code
		WHERE pe.docstatus=1 and pe.payment_type='Receive' and cs.customer_code="{}" and pe.payment_type='Receive' and MONTHNAME(si.posting_date)='{}' and YEAR(si.posting_date) = '{}' {}""".format(
            details,
            month,
            year,
            groupby,
