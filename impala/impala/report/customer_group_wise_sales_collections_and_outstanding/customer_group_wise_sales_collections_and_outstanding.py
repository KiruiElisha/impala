...
def execute(filters=None):
    ...
    for d in master:
        ...
        for i in range(12):
            ...
            groupby = "GROUP BY si.customer_group ORDER BY si.customer_group"
...
def get_details(filters):
    if filters.get("customer_groups"):
        details = frappe.db.sql(
            """SELECT name as details FROM `tabCustomer Group`
                WHERE name = '{}' """.format(
...
def get_monthly_sales(select_group, details, month, year, groupby):
    sales = frappe.db.sql(
        """SELECT {}, SUM(base_grand_total) as sales
                    ...
                    WHERE docstatus=1 and customer_group="{}" and MONTHNAME(posting_date)='{}' and YEAR(posting_date) = '{}' {}""".format(
            select_group,
            details,
            month,
            year,
...
def get_monthly_collections(details, month, year, groupby):
    collections = frappe.db.sql(
        """SELECT si.customer_group as details, SUM(pe.base_paid_amount) as collection
        ...
        WHERE pe.docstatus=1 and pe.payment_type='Receive' and si.customer_group="{}" and pe.payment_type='Receive' and MONTHNAME(si.posting_date)='{}' and YEAR(si.posting_date) = '{}' {}""".format(
            details,
            month,
            year,
            groupby,
