...
class ReceivablePayableReport(object):
	...
	def get_data(self):
		...
		data_cache_key = f"ar_ap_data_{self.filters.company}_{self.party_type}_{self.filters.report_date}"
		cached_data = frappe.cache().get_value(data_cache_key)
		...
		self.build_data()
		frappe.cache().set_value(data_cache_key, self.data, expires_in_sec=300)

	def init_voucher_balance(self):
		# build all keys, since we want to exclude vouchers beyond the report date
		for gle in self.gl_entries:
			
			party_pdcheques = frappe.db.sql("""select name, base_amount
			from `tabPosted Dated Cheques`
			where company='{}' and party = "{}" and posting_date <= '{}'
			""".format(self.filters.company, gle.party, self.filters.get("report_date")), as_dict=1)

			pdcheques = ""
			outstanding_amount = 0.0
			for pd in party_pdcheques:
				outstanding_amount += pd.base_amount
				pdcheques += str(pd.name)+ " ,"
	...
	def set_party_details(self, row):
		...
		if self.filters.get(scrub(self.filters.party_type)):
			...
			row.account_currency = row.account_currency
		else:
	...
	def get_gl_entries(self):
		...
		doc_currency_fields = (
			"debit as debit_in_account_currency, credit as credit_in_account_currency"
			if self.filters.get(scrub(self.party_type))
		...
		gl_entries = frappe.cache().get_value(f"gl_entries_{self.filters.company}_{self.party_type}")
		if not gl_entries:
			gl_entries = frappe.db.sql(
				f"""
				SELECT
					name, posting_date, account, party_type, party, voucher_type, voucher_no, cost_center,
					against_voucher_type, against_voucher, account_currency, {select_fields}, {doc_currency_fields} {remarks}
				FROM
					`tabGL Entry`
				WHERE
					docstatus < 2
			...
			frappe.cache().set_value(f"gl_entries_{self.filters.company}_{self.party_type}", gl_entries, expires_in_sec=300)
	...
	def add_common_filters(self, conditions, values, party_type_field):
		...
		if self.filters.department:
			...
			values.append(self.filters.department)
		...
		if self.filters.get("currency"):
			conditions.append("party in (SELECT name from `tabCustomer` where default_currency=%s)")
			values.append(self.filters.get("currency"))
	...
	def get_party_details(self, party):
		cache_key = f"party_details_{self.party_type}_{party}"
		party_details = frappe.cache().get_value(cache_key)

		if not party_details:
			if self.party_type == 'Customer':
				party_details = frappe.db.get_value('Customer', party, 
					['customer_name', 'territory', 'customer_group', 'customer_primary_contact'], 
					as_dict=True)
			else:
				party_details = frappe.db.get_value('Supplier', party, 
					['supplier_name', 'supplier_group'], 
					as_dict=True)
			
			frappe.cache().set_value(cache_key, party_details, expires_in_sec=3600)
	...
	def get_columns(self):
		columns_cache_key = f"ar_ap_columns_{self.party_type}_{self.filters.get('show_remarks')}_{self.filters.get('show_future_payments')}"
		cached_columns = frappe.cache().get_value(columns_cache_key)
		
		if cached_columns:
			self.columns = cached_columns
			return
		...
		self.add_column(_('Credit Note') if self.party_type == "Customer" else _('Debit Note'), fieldname='credit_note')
		...
		if self.filters.party_type == 'Customer':
			...
			self.add_column(label=_('Territory'), fieldname='territory', fieldtype='Link', options='Territory')
			self.add_column(label=_('Customer Group'), fieldname='customer_group', fieldtype='Link', options='Customer Group')
			if self.filters.show_sales_person:
				self.add_column(label=_('Sales Person'), fieldname='sales_person', fieldtype='Data')

		if self.filters.party_type == "Supplier":
			self.add_column(label=_('Supplier Group'), fieldname='supplier_group', fieldtype='Link', options='Supplier Group')

		frappe.cache().set_value(columns_cache_key, self.columns, expires_in_sec=3600)
