...
def get_data(filters):
	...
	accounts = frappe.db.sql("""select name, account_number, account_name, root_type, report_type, lft, rgt from `tabAccount` where company='Impala Glass Industries Ltd' AND is_group = 0 order by lft;""", filters.company, as_dict=True)
...
def get_rootwise_opening_balances(filters, report_type):
	...
	gle = frappe.db.sql("""
		...
		where
			...
			and account in (select name from `tabAccount` where report_type=%(report_type)s AND is_group = 0)
			and is_cancelled = 0
