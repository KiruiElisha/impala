...
def group_level_1_1(conditions = "" , grouping_level_1 = None,  grouping_level_value  = None, range_by = None , filters = None):
	if grouping_level_1:
		grouping = grouping_level_1.get("grouping")
		numbering = grouping_level_1.get("numbering")
		data =  frappe.db.sql(""" select  {} as level_1,  s.transaction_date, s.name  
			...
			IF(s.tax_type= 'Exclusive', (i.amount *  i.total_tax_percentage  / 100) + i.amount  , i.amount)   as amount_exclusive , 

			IF(s.tax_type= 'Inclusive',   i.amount -  (i.amount *  i.total_tax_percentage  / 100)   ,  0  ) as amount_inclusive , 
			...
			YEAR(s.transaction_date) as year  , s.status
			from `tabSales Order` s
			inner join 	`tabSales Order Item` i  on s.name = i.parent		
			inner join `tabSales Team` sp on s.name = sp.parent
	...
	if grouping_level_1:
		...
		if grouping_level_2:
			...
			(select sum(b.valuation_rate) from `tabBin` b where b.item_code = i.item_code) item_cost , 
 			...
 			i.amount as gross_profit ,
 			i.amount as gp,  s.territory , s.customer , s.project ,  s.cost_center as division  ,  s.status
...
def get_data_normal(conditions   = "", filters = ""):
		data =  frappe.db.sql(""" select  s.transaction_date, s.name  
			...
			as voucher_no , i.item_code ,  i.item_name ,   i.qty ,  s.status ,   i.amount as amount , sp.sales_person , 
			...
			i.amount as gross_profit ,
			i.amount as gp,  s.territory , s.customer , s.project ,  s.cost_center as division ,
			YEAR(s.transaction_date) as year  , s.tax_type ,  i.total_tax_percentage  , s.status
			from `tabSales Order` s
			inner join 	`tabSales Order Item` i  on s.name = i.parent		
...
def get_values_column( conditions = ""  , filters = ""):
	...
	if filters.get("value") and filters.get("value") != "":
		...
		if filters.get("value") == "Amount Exclusive":
			values+= " , IF(s.tax_type= 'Exclusive', sum((i.amount *  i.total_tax_percentage  / 100) + i.amount)  , sum(i.amount)  )  as value "

		if filters.get("value") == "Amount Inclusive":
			values+= """ ,
			IF(s.tax_type= 'Inclusive',   sum(i.amount -  (i.amount *  i.total_tax_percentage  / 100))   ,  0  ) as value """
