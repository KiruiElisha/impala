...
import erpnext
from impala.impala.report.balance_sheet_customized.balance_sheet_customized import (
	get_chart_data,
...
from impala.impala.report.profit_and_loss_statement_customized.profit_and_loss_statement_customized import (
...
def execute(filters=None):
	...
	if filters.get('report') == "Balance Sheet":
		data, message, chart, report_summary = get_balance_sheet_data(conversion_factor, fiscal_year, companies, columns, filters)
	elif filters.get('report') == "Profit and Loss Statement":
		data, message, chart, report_summary = get_profit_loss_data(conversion_factor, fiscal_year, companies, columns, filters)
	else:
		if cint(frappe.db.get_single_value('Accounts Settings', 'use_custom_cash_flow')):
			from impala.impala.report.cash_flow_customized.cash_flow_customized import execute as execute_custom
			return execute_custom(filters=filters)

		data, report_summary = get_cash_flow_data(conversion_factor, fiscal_year, companies, filters)

	return columns, data, message, chart, report_summary

def get_balance_sheet_data(conversion_factor, fiscal_year, companies, columns, filters):
	asset = get_data(conversion_factor, companies, "Asset", "Debit", fiscal_year, filters=filters)

	liability = get_data(conversion_factor, companies, "Liability", "Credit", fiscal_year, filters=filters)

	equity = get_data(conversion_factor, companies, "Equity", "Credit", fiscal_year, filters=filters)
	...
	provisional_profit_loss, total_credit = get_provisional_profit_loss(asset, liability, equity,
		companies, filters.get('company'), conversion_factor, company_currency, True)
	...
	report_summary = get_bs_summary(companies, asset, liability, equity, provisional_profit_loss, total_credit,
		company_currency, conversion_factor, filters, True)

	chart = get_chart_data(filters, columns, asset, liability, equity, conversion_factor)
...
def get_profit_loss_data(conversion_factor, fiscal_year, companies, columns, filters):
	income, expense, net_profit_loss = get_income_expense_data(conversion_factor, companies, fiscal_year, filters)
	company_currency = get_company_currency(filters)
...
def get_income_expense_data(conversion_factor, companies, fiscal_year, filters):
	company_currency = get_company_currency(filters)
	income = get_data(conversion_factor, companies, "Income", "Credit", fiscal_year, filters, True)

	expense = get_data(conversion_factor, companies, "Expense", "Debit", fiscal_year, filters, True)

	net_profit_loss = get_net_profit_loss(income, expense, companies, filters.company, conversion_factor, company_currency, True)
...
def get_data(conversion_factor, companies, root_type, balance_must_be, fiscal_year, filters=None, ignore_closing_entries=False):
	...
	calculate_values(conversion_factor, accounts_by_name, gl_entries_by_account, companies, filters, fiscal_year)
...
def calculate_values(conversion_factor, accounts_by_name, gl_entries_by_account, companies, filters, fiscal_year):
			...
			if d:
				...
				for company in companies:
					...
					if (entry.company == company or (filters.get('accumulated_in_group_company'))
						...
						debit, credit = flt(entry.debit)/conversion_factor, flt(entry.credit)/conversion_factor
