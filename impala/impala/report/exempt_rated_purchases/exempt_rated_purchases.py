...
def get_data(conditions):
    ...
    data = frappe.db.sql(
        """select s.bill_no as supp_inv, s.bill_date as supp_inv_date, c.tax_id as pin_no, s.supplier_name as supplier_name, s.posting_date as posting_date, s.name as name,
			...
			where s.docstatus=1 and c.unregistered=0 and i.item_tax_template IN ('Exempt - IG','Exempt - IAL') {} group by s.name order by s.posting_date DESC""".format(
            conditions
        ),
        as_dict=1,
    )
