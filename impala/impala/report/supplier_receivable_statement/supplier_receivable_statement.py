...
def execute(filters=None):
	...
	if ageing:
		ageing_data["30"] = ageing.get("30")
		ageing_data["60"] = ageing.get("60")
		ageing_data["90"] = ageing.get("90")
		ageing_data["120"] = ageing.get("120")
		ageing_data["above"] = ageing.get("above")
		ageing_data["30"] = ageing.get("30")
		ageing_data["30"] = ageing.get("30")
	...
	if filters.get("party_type") and filters.get('party'):
		...
		if filters.get("party_type") == "Supplier":
			...
			customer_detail = frappe.db.get_value("Supplier" , {"name" : party }, 
				["supplier_name" , "supplier_primary_address" , "company"] , as_dict = 1 )
			if customer_detail:
				customer_personal_detial["customer_name"] = customer_detail.get("supplier_name")
				if customer_detail.get("supplier_primary_address"):
					address_doc = frappe.get_doc("Address" , customer_detail.get("supplier_primary_address"))


		elif filters.get("party_type") == "Customer":
			customer_detail = frappe.db.get_value("Customer" , {"name" : party }, 
		...
		if customer_detail:
			...
			customer_personal_detial["currency"] =  frappe.db.get_value("Company" , {"name" : filters.get("company") } , "default_currency") 
...
def get_supplier_customer_sales_return(filters):
	...
	sales_return = frappe.db.sql( """ SELECT sum(grand_total) as total_return 
		...
		{} """.format(doctype_name , conditions),as_dict=1, debug = 1)
	if sales_return:
...
def get_pd_cheque_detail(filters):
	...
	if filters.get("party_type") and filters.get('party'):
		...
		cheque_list = frappe.db.sql(""" 
			select pd.name as pd_cheque_name , pd.posting_date as pd_posting_date ,   
			...
			where 1=1 and pd.party_type = '{}' and pd.party = '{}' and pd.posting_date >= '{}'

		 """.format(filters.get("party_type") , party , getdate(nowdate()) ), as_dict = 1, debug = 1)
...
def get_chart(filters):
	...
	if filters.get("party_type") and filters.get("party"):
		...
		if filters.get("party_type") == "Customer":
			doctype_name = "Sales Invoice"
			conditions += "and customer =  '{}' ".format(filters.get("party_type") ,  party)
			payment_conditions += "and party_type = '{}'  and party =  '{}' ".format(filters.get("party_type") ,  party)

		elif filters.get("party_type") == "Supplier":
			doctype_name = "Purchase Invoice"
			conditions += " and supplier =  '{}' ".format(filters.get("party_type") ,  party)
			payment_conditions += "and party_type = '{}'  and party =  '{}' ".format(filters.get("party_type") ,  party)
	...
	for mon in month:
		sales = frappe.db.sql(
			...
			WHERE
				...
				""".format(doctype_name , mon , filters.get("to_date").split("-")[0] , conditions),
			as_dict=1, debug = 1
		)
		if sales:
...
def get_ageing(filters):
	...
	if filters.get("party_type") and filters.get("party"):
		...
		if filters.get("party_type") == "Customer":
			doctype_name = "Sales Invoice"
			sales_conditions += "and customer =  '{}' ".format(party)

		elif filters.get("party_type") == "Supplier":
			doctype_name = "Purchase Invoice"
			sales_conditions += "and supplier =  '{}' ".format(party)
	...
	ageing = frappe.db.sql(
		""" select posting_date, sum(outstanding_amount) as outstanding from 
		`tab{}` where docstatus=1 and is_pos=0 and  outstanding_amount != 0 {} 
		group by posting_date order by posting_date    """.format(doctype_name , sales_conditions),
		as_dict=1, debug = 1)


	from datetime import datetime
	if ageing:
		d_to_date = datetime.strptime(str(getdate(nowdate())), "%Y-%m-%d")
		if d_to_date:
			for d in ageing:
