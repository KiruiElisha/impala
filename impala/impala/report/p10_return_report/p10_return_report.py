...
def execute(filters=None):
	...
	employee_list = frappe.db.sql("""select e.national_id, e.name, e.employee_name, e.residential_status, e.employee_pin, e.type_of_employee
		from `tabEmployee` e inner join `tabSalary Slip` ss on ss.employee = e.name where ss.docstatus != 2 
		{}  group by 1 order by 1""".format(conditions), as_dict=1)
	
	for employee in employee_list:
		row = {}
		row['national_id'] = employee.national_id
		...
		for slip in salary_slips:
			...
			if salary_slip_details:
				for s in salary_slip_details:
					...
					elif s.salary_component=='Total NSSF':
...
def get_salary_details_for_employee(filters, conditions, employee, slip):
	...
	return frappe.db.sql("""select sd.salary_component, sd.amount, 
		...
		WHERE ss.name='%s' and ss.employee='%s' %s
