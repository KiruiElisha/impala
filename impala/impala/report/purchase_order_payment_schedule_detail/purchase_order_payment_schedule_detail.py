...
def get_data(conditions = ""):
	data =  frappe.db.sql(""" select  s.company , s.name as purchase_order , s.status , s.supplier ,  s.transaction_date ,
		i.amount , i.assign , i.base_amount , i.creation , i.currency , i.es , i.ex_rate , i.notification_date  ,  i.paid ,
		i.status as child_status , i.task
		from `tabPurchase Order` s
		inner join 	`tabPurchase Order Payment Schedule` i  on s.name = i.parent
