    ...
    if filters.get("un_group") == 1:
        ...
        for currency in currency_groups:
            ...
            for account in accounts:
                ...
                # frappe.msgprint(str(account_row))

                against_accounts = get_account_wise_data_ungrouped(
                ...
                for against in against_accounts:
                    ...
                    against_row["against"] = against.name
        ...
        for currency in currency_groups:
            ...
            for account in accounts:
                ...
                # frappe.msgprint(str(against_accounts))
...
def get_account_wise_data(account, currency, gle_conditions):
    data = frappe.db.sql(
        """
			SELECT ac.parent_account, ac.name, gl.against, SUM(gl.debit_in_account_currency) as transaction_flowin, SUM(gl.credit_in_account_currency) as transaction_flowout,
...
def get_account_wise_data_ungrouped(account, currency, gle_conditions):
    data = frappe.db.sql(
        """
			SELECT ac.parent_account,ac.name, gl.against, SUM(gl.debit_in_account_currency) as transaction_flowin, SUM(gl.credit_in_account_currency) as transaction_flowout,
			SUM(gl.debit) as base_flowin, SUM(gl.credit) as base_flowout, ac.is_group as is_group 
			FROM `tabGL Entry` gl
			INNER JOIN `tabAccount` ac on ac.name = gl.against 
			WHERE ac.is_group=0 and gl.docstatus=1 and gl.account = "{}" and gl.account_currency='{}' {} GROUP BY ac.name ORDER BY ac.name 
		""".format(
            account, currency, gle_conditions
        ),
        as_dict=1,
