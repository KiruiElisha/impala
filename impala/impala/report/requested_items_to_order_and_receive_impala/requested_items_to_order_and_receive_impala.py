...
def get_conditions(filters):
	...
	if filters.get("item_group"):
		conditions += " and mr_item.item_group = '{0}'".format(filters.get("item_group"))

	if filters.get("supplier"):
		conditions += " and supplier = '{0}'".format(filters.get("supplier"))

	if filters.get("supplier_group"):
		conditions += " and supplier_group = '{0}'".format(filters.get("supplier_group"))

	if filters.get("project"):
		conditions += " and mr_item.project = '{0}'".format(filters.get("project"))

	if filters.get("department"):
		conditions += " and mr_item.department = '{0}'".format(filters.get("department"))

	if filters.get("cost_center"):
		conditions += " and mr_item.cost_center = '{0}'".format(filters.get("cost_center"))
...
def get_data(filters, conditions):
	data = frappe.db.sql("""
		select
			...
			mr_item.department as department,
			mr_item.cost_center as cost_center,
			mr.company as company,
			(select supplier from `tabPurchase Order` po inner join `tabPurchase Order Item` poi on po.name=poi.parent where material_request=mr.name) as supplier,
			(select supplier_group from `tabSupplier` s where name=supplier) as supplier_group
