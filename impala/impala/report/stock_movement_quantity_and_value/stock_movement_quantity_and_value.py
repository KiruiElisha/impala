...
def get_columns(filters):
    columns = [
		...
		{"label": _("Opening Value"), "fieldname": "opening_val", "fieldtype": "Currency", "width": 140, "options": "currency"},
		{"label": _("In Qty"), "fieldname": "in_qty", "fieldtype": "Float", "width": 100, "convertible": "qty"},
		{"label": _("In Value"), "fieldname": "in_val", "fieldtype": "Float", "width": 140},
		{"label": _("Out Qty"), "fieldname": "out_qty", "fieldtype": "Float", "width": 100, "convertible": "qty"},
		{"label": _("Out Value"), "fieldname": "out_val", "fieldtype": "Float", "width": 140},
		{"label": _("Closing Qty"), "fieldname": "bal_qty", "fieldtype": "Float", "width": 100, "convertible": "qty"},
		{"label": _("Closing Value"), "fieldname": "bal_val", "fieldtype": "Currency", "width": 140, "options": "currency"},
    ]
...
def get_stock_ledger_entries(filters, items):
	...
	return frappe.db.sql("""
		select
			sle.item_code, sle.actual_qty, sle.valuation_rate, sle.posting_date,
			sle.company, sle.qty_after_transaction, sle.stock_value_difference,
			sle.item_code as name, sle.voucher_no, sle.stock_value, sle.batch_no
		from
			`tabStock Ledger Entry` sle inner join `tabStock Entry` se on sle.voucher_no = se.name
		where sle.docstatus < 2 and sle.voucher_type = 'Stock Entry' and se.stock_entry_type not in ('Material Transfer','Damage Transfer','Material Transfer for Manufacture') %s %s
		and is_cancelled = 0 group by sle.item_code 
