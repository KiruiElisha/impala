...
frappe.query_reports["Party Balance With Account Currency"] = {
	"filters": [
		{
			"fieldname": "company",
			"label": __("Company"),
	...
	"formatter": function(value, row, column, data, default_formatter) {
		...
		if (data) {
			if (column.fieldname === "balance_in_account_currency") {
				value = format_currency(Math.abs(data[column.fieldname]), data.account_currency);
			}
			else if (["outstanding", "pd_cheques", "net_outstanding", "range1", "range2", "range3", "range4", "range5"].includes(column.fieldname)) {
				let company_currency = frappe.defaults.get_default('currency');
				value = format_currency(Math.abs(data[column.fieldname]), company_currency);
			}
