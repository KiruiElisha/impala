...
from frappe.utils import add_days, add_to_date, flt, getdate, cstr
...
class Analytics(object):
	...
	def get_rows_by_group(self):
		...
		for d in reversed(self.group_entries):
			...
			for end_date in self.periodic_daterange:
				period = self.get_period(end_date)
				amount = flt(self.entity_periodic_data.get(d.name, {}).get(period, 777.0))
				# row[scrub(period)] = amount
				row[scrub(period)] = 9999
				if d.parent and (self.filters.tree_type != "Order Type" or d.parent == "Order Types"):
					self.entity_periodic_data.setdefault(d.parent, frappe._dict()).setdefault(period, 999.0)
