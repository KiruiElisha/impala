...
def execute(filters=None):
    ...
    if filters.get("age1") and filters.get("age2") and filters.get("age1") > filters.get("age2"):
        frappe.throw("Age 1 should be less than Age 2")
    if filters.get("age2") and filters.get("age3") and filters.get("age2") > filters.get("age3"):
        frappe.throw("Age 2 should be less than Age 3")
    if filters.get("age3") and filters.get("age4") and filters.get("age3") > filters.get("age4"):
...
def get_customer_data(filters):
    ...
    if filters.get("customer"):
        filt += " and customer = '{0}'".format(filters.get("customer"))
    ...
    if filters.get("from_date"):
        opening = frappe.db.sql(
            """ select sum(outstanding_amount) as outstanding from `tabSales Invoice` where docstatus=1  and outstanding_amount>0 and posting_date <'{1}' and  company='{2}' {0} """.format(
                filt, filters.get("from_date"), filters.get("company")
            ),
            as_dict=True,
        )
        ...
        opening_payment = frappe.db.sql(
            """ select sum(paid_amount) as paid_amount from `tabPayment Entry` where docstatus=1 and payment_type='Receive' and paid_amount>0 and party='{0}' and posting_date<'{1}' and company='{2}' """.format(
                filters.get("customer"), filters.get("from_date"), filters.get("company")
            ),
            as_dict=True,
        )
        ...
        a_y = from_date.split("-")[2]
        a_m = from_date.split("-")[1]
        a_d = from_date.split("-")[0]
        ...
        from_date = datetime.strptime(str(c), "%Y-%m-%d").date()
        ...
        if min_date:
            ...
            if filters.get("to_date"):
                from_date = datetime.strptime(str(filters.get("to_date")), "%Y-%m-%d").date()
    if filters.get("to_date"):
        ...
        c = a_y + "-" + a_m + "-" + a_d
        frappe.log_error(c)
        to_date = datetime.strptime(str(c), "%Y-%m-%d").date()
    ...
    pe_filt = ""
    if filters.get("customer"):
        si_filt += " and customer='{0}'".format(filters.get("customer"))
        pe_filt += " and party='{0}'".format(filters.get("customer"))
    ...
    for i in range(period.days + 1):
        ...
        si_data = frappe.db.sql(
            """ select posting_date, customer, name, rounded_total, grand_total from `tabSales Invoice` where docstatus=1 and is_pos=0 and posting_date='{0}' and company='{2}' {1} """.format(
                date, si_filt, filters.get("company")
            ),
            as_dict=1,
        )
        for j in si_data:
        ...
        pe_data = frappe.db.sql(
            """ select posting_date, party, name, paid_amount from `tabPayment Entry` where docstatus=1 and payment_type="Receive" and posting_date='{0}' and company='{2}' {1} """.format(
                date, pe_filt, filters.get("company")
            ),
            as_dict=1,
        )
        for j in pe_data:
    ...
    filtr = ""
    if filters.get("from_date"):
        filtr += " and posting_date >= '{0}'".format(filters.get("from_date"))
    if filters.get("customer"):
        filtr += " and customer = '{0}'".format(filters.get("customer"))
    ageing = frappe.db.sql(
        """ select posting_date, sum(outstanding_amount) as outstanding from `tabSales Invoice` where docstatus=1 and is_pos=0 and outstanding_amount>0 and posting_date<='{0}' and company='{2}' {1} group by posting_date""".format(
            filters.get("to_date"), filtr, filters.get("company")
        ),
        as_dict=1, 
    )
    ...
    dic = {
        ...
        filters.get("age4"): 0,
        "Above": 0,
        "age1": filters.get("age1"),
        "age2": filters.get("age2"),
        "age3": filters.get("age3"),
        "age4": filters.get("age4"),
        "age5": "Above",
        "bal": data[-1]["balance"] if len(data) and "balance" in data[-1] else 0,
        "customer_name": frappe.db.get_value(
            "Customer", filters.get("customer"), "customer_name"
        ),
    }
    for i in data:
        if "date" in i:
            age = get_age(i["date"], filters.get("to_date"), filters)
            if "debit" in i:
                dic[age] += i["debit"]
            if "credit" in i:
                dic[age] -= i["credit"]
    ...
    if filters.get("customer"):
        dic["address"] = frappe.db.get_value(
            "Customer", filters.get("customer"), "primary_address"
        )
    data.append(dic)
    return data
