...
class ReceivablePayableReport(object):
	...
	def get_data(self):
		cache_key = f"receivable_payable_report:{self.filters.company}:{self.filters.report_date}"
		cached_data = redis_client.get(cache_key)
		if cached_data:
			self.data = frappe.parse_json(cached_data)
		else:
			self.get_gl_entries()
			...
			redis_client.setex(cache_key, 3600, frappe.as_json(self.build_data()))
	...
	def build_data(self):
		...
		if self.filters.get('group_by_party'):
			yield self.get_subtotal_row(self.previous_party)
			if self.data:
				yield self.total_row_map.get('Total')
	def get_row(self, row):
		...
		return row
	def get_subtotal_row(self, party):
		if party in self.total_row_map:
			return self.total_row_map[party]
		return {}
	...
	def get_gl_entries(self):
		...
		cache_key = f"gl_entries:{self.filters.company}:{self.filters.report_date}:{hash(conditions)}"
		cached_entries = redis_client.get(cache_key)
		if cached_entries:
			self.gl_entries = frappe.parse_json(cached_entries)
		else:
		...
		yield from self.gl_entries
		redis_client.setex(cache_key, 3600, frappe.as_json(self.gl_entries))  # Cache for 1 hour
	...
	def get_party_details(self, party):
		cache_key = f"party_details:{self.party_type}:{party}"
		cached_details = redis_client.get(cache_key)
		if cached_details:
			return frappe.parse_json(cached_details)
		...
		redis_client.setex(cache_key, 3600, frappe.as_json(self.party_details[party])) 
