...
def calculate_values(accounts, gl_entries_by_account, opening_balances, filters, company_currency):
	...
	for d in accounts:
		...
		# add opening
		d["opening_debit"] = opening_balances.get(d.name, {}).get("opening_debit", 0)/conversion_factor
		d["opening_credit"] = opening_balances.get(d.name, {}).get("opening_credit", 0)/conversion_factor

		for entry in gl_entries_by_account.get(d.name, []):
			if cstr(entry.is_opening) != "Yes":
				d["debit"] += flt(entry.debit)/conversion_factor
				d["credit"] += flt(entry.credit)/conversion_factor

		d["closing_debit"] = (d["opening_debit"] + d["debit"])/conversion_factor
		d["closing_credit"] = (d["opening_credit"] + d["credit"])/conversion_factor
